...
import { z } from 'zod'
import type { IFilter } from '~/@core/fields/interfaces/IFieldInterfaces'
import type { FieldDefinitionType } from '~/@core/form/EditFormV3'
import type { IColumn } from '~/@core/table/interfaces/ITableInterfaces'
import { E_BOOLEAN, E_STATUS, E_STATUS_TABLE } from '~/utils/Enums'
...
export function statusForTable(status: string): string {
  if (status in E_STATUS) {
    return E_STATUS[status as keyof typeof E_STATUS] === E_STATUS.ACTIVE ? E_STATUS_TABLE.ACTIVE : E_STATUS_TABLE.INACTIVE
  }
  return ''
}
...
export function boolToString(isTrue: boolean): string {
  return isTrue ? E_BOOLEAN.IS_TRUE : E_BOOLEAN.IS_FALSE
}
