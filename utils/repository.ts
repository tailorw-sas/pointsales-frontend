...
import type { IQueryRequest } from '~/@core/fields/interfaces/IFieldInterfaces'

export function repository<T>(fetch: $Fetch<T, NitroFetchRequest>) {
  return {
    async search<T>(MODULE_NAME: string, URI_API: string, payload: IQueryRequest): Promise<T> {
    ...
    async sendCodeOtpLogged(payload: IQueryRequest) {
    ...
    async changePasswordLogged(payload: IQueryRequest) {
    ...
    async adminChangePassword(payload: IQueryRequest) {
      const url = `/users/admin/change-password`
      return fetch(url, {
        method: 'POST',
        body: payload
      })
