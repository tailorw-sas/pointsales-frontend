...
async function getListMeijetConfigOnDemand(query: string = '') {
  try {
    const payload
        = {
          filter: [
    ...
    listConfigMailjet.value = []
    for (const iterator of dataList) {
      listConfigMailjet.value = [...listConfigMailjet.value, { id: iterator.id, name: iterator.name, status: iterator.status }]
    }
  }
  catch (error) {
    console.error('Error on loading mailjet config list:', error)
  }
}
...
async function save(item: { [key: string]: any }) {
  ...
  payload.templateCode = payload.templateCode.trim()
        ...
        >
          <template #field-mailjetConfigId="{ item: data, onUpdate }">
            <DebouncedAutoCompleteComponent
              ...
              field="name"
              item-value="id"
              :model="data.mailjetConfigId"
              placeholder="Escriba un nombre o email para buscar."
              :suggestions="listConfigMailjet"
              @change="($event) => {
                onUpdate('mailjetConfigId', $event)
              }"
              @load="($event) => getListMeijetConfigOnDemand($event)"
            />
