...
const fields: Array<FieldDefinitionType> = [
  {
    ...
    headerClass: 'mb-1',
    validation: z.object({
      id: z.string().min(1, 'El campo módulo es requerido'),
      name: z.string().min(1, 'El campo módulo es requerido'),
    }).nullable().refine(value => value && value.id && value.name, { message: 'El campo módulo es requerido' }),
  },
  {
    field: 'action',
    header: 'Acción',
    ...
    hidden: false,
    headerClass: 'mb-1',
    validation: z.string().trim().min(1, 'El campo acción es requerido').max(50, 'Máximo 50 caracteres')
  },
  // {
  //   field: 'action',
  //   header: 'Acción',
  ...
  //   headerClass: 'mb-1',
  //   validation: z.object({
  //     id: z.string().min(1, 'El campo acción es requerido'),
  //     name: z.string().min(1, 'El campo acción es requerido'),
  //   }).nullable().refine(value => value && value.id && value.name, { message: 'El campo acción es requerido' }),
  // },
  {
    field: 'code',
    header: 'Código',
    ...
    class: 'field col-12 required',
    headerClass: 'mb-1',
    validation: z.string().trim().min(1, 'El campo código es requerido').max(250, 'Máximo 250 caracteres')
  },
  {
    field: 'description',
    header: 'Descripción',
...
const columns: IColumn[] = [
  { field: 'moduleName', header: 'Módulo', type: 'text' },
  { field: 'action', header: 'Acción', type: 'text' },
...
const options = ref({
  ...
  messageToDelete: '¿Estás seguro que deseas eliminar el permiso: {{name}}?'
})
...
function searchAndFilter() {
  ...
  if (filterToSearch.value.criterial && moduleSelected.value) {
    payload.value.filter = [...payload.value.filter, {
      key: filterToSearch.value.criterial ? filterToSearch.value.criterial.id : '',
      operator: 'IN',
      value: moduleSelected.value.map((item: any) => item.id),
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.moduleId = moduleList.value.find(item => item.id === response.module.id)
        item.value.action = response.action
...
async function getModuleList(query: string = '') {
  try {
    const payload
        = {
          filter: [
    ...
    moduleList.value = []
    for (const iterator of dataList) {
      moduleList.value = [...moduleList.value, { id: iterator.id, name: iterator.name }]
    }
  }
  catch (error) {
...
async function parseDataTableFilter(payloadFilter: any) {
  ...
  const moduleName = parseFilter?.find((item: IFilter) => item?.key === 'moduleName')

  if (moduleName) {
    moduleName.key = 'module.name'
  }
...
function onSortField(event: any) {
  if (event) {
    ...
    if (event) {
      if (event.sortField === 'moduleName') {
        event.sortField = 'module.name'
      }
    }
...
const disabledClearSearch = computed(() => {
  return !(filterToSearch.value.criterial || (filterToSearch.value.search || moduleSelected.value.length > 0))
})
// -------------------------------------------------------------------------------------------------------
...
<template>
  <div class="flex justify-content-between align-items-center">
    ...
    <IfCan :perms="['ADMINISTRACION:GESTION-PERMISOS']">
            ...
            <div class="flex gap-4 flex-column lg:flex-row">
              ...
              <div class="flex align-items-center gap-2">
                ...
                <DebouncedAutoCompleteComponent
                  ...
                  item-value="id"
                  multiple
                  :model="moduleSelected"
                  :suggestions="moduleList"
                  @change="($event) => {
                    moduleSelected = $event
                  }"
                  @load="($event) => getModuleList($event)"
                />
      ...
      <div class="bg-white">
        ...
        <div class="card">
          <EditFormV3
            ...
            :permission-save="idItem ? ['ADMINISTRACION:GESTION-PERMISOS'] : ['ADMINISTRACION:GESTION-PERMISOS']"
            :permission-delete="['ADMINISTRACION:GESTION-PERMISOS']"
            @cancel="clearForm"
            ...
            <!-- <template #field-action="{ item: data, onUpdate }">
              <Dropdown
                ...
                @update:model-value="($event) => {
                  ...
                  onUpdate('description', `${data.moduleId ? `${data.moduleId.name}:` : ''}${$event ? $event.name : ''}`)
                }"
              />
              <Skeleton v-else height="2rem" class="mb-2" />
            </template> -->
