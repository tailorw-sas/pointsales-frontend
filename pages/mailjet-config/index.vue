...
const fields: Array<FieldDefinitionType> = [
  ...
  },
  {
    ...
    headerClass: 'mb-1',
    validation: z.string().trim().min(1, 'El campo correo electrônico es requerido').max(50, 'Máximo 50 caracteres').email('Correo inválido')
  },
  {
    field: 'mailjetApiKey',
    header: 'Api Key',
    ...
    class: 'field col-12 required',
    headerClass: 'mb-1',
    validation: z.string().min(1, 'El campo api key es requerido').max(20, 'Máximo 20 caracteres')
  },
  {
    ...
    class: 'field col-12 required',
    headerClass: 'mb-1',
    validation: z.string().min(1, 'El campo api secret es requerido').max(50, 'Máximo 50 caracteres')
  },
]
...
const columns: IColumn[] = [
  { field: 'fromName', header: 'Nombre', type: 'text' },
  { field: 'fromEmail', header: 'Correo Electrónico', type: 'text' },
  { field: 'createdAt', header: 'Fecha de Creación', type: 'date' },
]
// -------------------------------------------------------------------------------------------------------
const ENUM_FILTER = [
  { id: 'fromName', name: 'Nombre' },
  { id: 'fromEmail', name: 'Correo Electrónico' },
]
// TABLE OPTIONS -----------------------------------------------------------------------------------------
const options = ref({
  ...
  try {
    ...
    for (const iterator of dataList) {
      if (!existingIds.has(iterator.id)) {
        const date = new Date(iterator.createdAt)
        newListItems.push({ ...iterator, fromName: iterator.name, fromEmail: iterator.email, createdAt: date, loadingEdit: false, loadingDelete: false })
  ...
  if (id) {
    ...
    try {
      ...
      if (response) {
        ...
        item.value.tenant = response.tenant
      }
      ...
      if (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Configuración no pudo ser cargada', life: 3000 })
      }
    }
    finally {
...
async function createItem(item: { [key: string]: any }) {
  if (item) {
    ...
    const obj = { name: payload.name, email: payload.email, mailjetApiKey: payload.mailjetApiKey, mailjetApiSecret: payload.mailjetApiSecret, tenant: currentBussiness.value.businessId }
    await GenericService.create(confApi.moduleApi, confApi.uriApi, obj)
  }
}
...
async function updateItem(item: { [key: string]: any }) {
  ...
  const payload: { [key: string]: any } = { ...item }
  const obj = { name: payload.name, email: payload.email, mailjetApiKey: payload.mailjetApiKey, mailjetApiSecret: payload.mailjetApiSecret, tenant: currentBussiness.value.businessId }
  await GenericService.update(confApi.moduleApi, confApi.uriApi, idItem.value || '', obj)
}
...
<template>
  <div class="flex justify-content-between align-items-center">
    ...
    <IfCan :perms="['MAILJET_CONFIG:CREATE']">
