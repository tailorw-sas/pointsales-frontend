...
function addItemToListServices() {
  ...
  if (countError === 0) {
    ...
    itemsServicesListTable.value = [...itemsServicesListTable.value, { ...payload, id: formConfigService.value.service.value.id, service: formConfigService.value.service.value.name, price: formConfigService.value.price.value }]
    pagination.value.totalElements = itemsServicesListTable.value.length
    formConfigService.value.service.value = ''
  ...
  try {
    ...
    for (const iterator of dataList) {
      if (!existingIds.has(iterator.service.id)) {
        newListItems.push({ ...iterator, id: iterator.service.id, service: iterator.service.name, loadingEdit: false, loadingDelete: false })
...
async function assignServices() {
  try {
    ...
    if (itemsServicesListTable.value.length > 0) {
      ...
      await GenericService.updateWithOutId('shift', 'business-services', payloadData)
      goToList()
    }
  }
  catch (error) {
...
async function parseDataTableFilter(payloadFilter: any) {
  const parseFilter: IFilter[] | undefined = await getEventFromTable(payloadFilter, columnsServices)
        ...
        <div key="formReceipt" class="card pb-0">
          <div class="grid p-fluid formgrid p-0">
            ...
            <div class="col-12 md:col-2 field flex justify-content-end align-items-end">
              ...
              <Button v-if="true" class="w-3rem h-2rem mx-1 flex align-items-center justify-content-center" label="" severity="secondary" @click="cancelService">
