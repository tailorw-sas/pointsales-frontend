...
const emit = defineEmits(['close', 'save', 'update:formConfig', 'forceSave', 'haveErrorInForm'])
...
async function getCurrentProfile() {
  try {
    if (props.formOptions.moduleApi && props.formOptions.uriApi) {
      loadingForm.value = true

      const response = await GenericService.getProfile(props.formOptions.moduleApi, props.formOptions.uriApi, 'me')
    ...
    if (props.formOptions.moduleApi && props.formOptions.uriApi && props.formOptions.itemId) {
      ...
      for (const key in objModelValue) {
        ...
        if (objModelValue.hasOwnProperty(key)) {
          ...
          if ((objModelValue[key].type === 'image' || objModelValue[key].type === 'file')) {
            if (objModelValue[key].value.value) {
  ...
  <div class="p-4">
    ...
    <div v-if="formOptions.hasOwnProperty('showSecondGrid') ? formOptions.showSecondGrid : true" class="grid p-fluid formgrid">
      <div v-for="(objField, index) of objModelValue" :key="index" :class="`col-12 lg:col-${objField.col}`" class="field">
        ...
        />
        <SelectFieldComponnent
          v-if="!listKeyFormWithImage.includes(index.toString()) && (objField.type === 'select')"
          :key="refKey"
          :properties="objField"
          :submit="submitForm"
        ...
        />
        <SearchSelectFieldComponnent
          v-if="!listKeyFormWithImage.includes(index.toString()) && (objField.type === 'select-search')"
          :key="refKey"
          :properties="objField"
          :submit="submitForm"
          :local-items="objField.localItems"
    ...
    <div v-if="props.formOptions.hasOwnProperty('showBtnActions') ? props.formOptions.showBtnActions : true" class="grid p-fluid formgrid">
      <div class="col-12 flex justify-content-end mt-4">
        <Button v-if="props.formOptions.hasOwnProperty('showBtnCancel') ? props.formOptions.showBtnCancel : true" class="w-8rem mr-3" severity="secondary" icon="pi pi-times" :label="props.formOptions.btnTextCancel ? props.formOptions.btnTextCancel : 'Cancel'" @click="onClose()" />
        <Button v-if="props.formOptions.hasOwnProperty('showBtnSave') ? props.formOptions.showBtnSave : true" class="w-8rem" icon="pi pi-check" :label="props.formOptions.btnTextSave ? props.formOptions.btnTextSave : 'Save'" :loading="loadingForm" :disabled="disabledForm" @click="saveItem()" />
      </div>
    </div>
  </div>
</template>
