...
import { type PropType, onUpdated, ref, watch, watchEffect } from 'vue'
import { maxLength, minLength } from '../utils/valid_form_fields'
...
function handleInput(event?: Event) {
  ...
  for (const keyword of props.validationKeywords) {
    if (keyword in methods.value) {
      ...
      if (keyword.includes('maxLength')) {
        // const cant = keyword.split(':')[1] para el caso de que quiera poner la cantidad de caracteres por el mismo keyword y no por la props
  ...
  emits('update:modelValue', fieldModelValue.value.trim().toUpperCase())
...
watch(() => props.properties.value, (newValue) => {
  fieldModelValue.value = newValue
})
...
<template>
  <div>
    ...
    <div v-if="props.properties.showCounter && (props.properties.type === 'textarea' || props.properties.type === 'number')" class="flex justify-content-end py-0 my-0" style="font-size: 10px;">
