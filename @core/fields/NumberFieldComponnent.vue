...
import { maxLength, minLength, onlyNumbers, validEmail } from '../utils/valid_form_fields'
...
function handleInput(value: number) {
  ...
  for (const keyword of props.validationKeywords) {
    if (keyword in methods.value) {
      let error = ''
      if (keyword.includes('maxLength:') || keyword.includes('minLength:')) {
        const cant = keyword.split(':')[1]
  ...
  <div>
    ...
    <div>
      ...
      <InputNumber
        ...
        size="large"
        @input="handleInput(fieldModelValue)"
      />
    </div>
