...
<template>
  <div>
    ...
    <div>
      <MultiSelect
        :id="fieldId" v-model="fieldModelValue" :options="listItems.length !== 0 ? listItems : localItems"
        option-label="name" size="large" :required="props.properties.isRequired"
        :placeholder="props.properties.placeholder" :class="{ 'p-invalid': listError.length !== 0 }" autocomplete="off"
        class="text-field-size h-2rem align-items-center" filter @change="handleInput"
      >
    ...
    <div :class="props.properties.showCounter ? 'margin-counter' : ''">
      <small :id="`${fieldId}-help`" class="p-error text-start">
        <transition-group name="error-fade">
          <span v-for="textError of listError" :key="textError.key" class="text-error block flex pl-2 mb-0">{{
